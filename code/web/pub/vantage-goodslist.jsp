	...
	<body>
		<div class="categoryPagecontantRight">
			...
			<div class="categoryPagecontantRight2">
			  	<div class="orderDiv" id='vantage_orderDiv'>
				  	<!-- span class="orderBy">排序：</span>
					<span class="buttonInitClassLeft" id='btnSale' onclick="_vantageApp.sort('0');">
						销  量
					</span>
					<span class="buttonInitClassLeft" id='btnPrice' onclick="_vantageApp.sort('1');">
						价  格
					</span>
					<span class="buttonInitClassLeft" id="btnDate" onclick="_vantageApp.sort('2');">
						上架时间
					</span> -->
			  	</div>
...
function VantageApp() {
	...
	var load = function(pageIndex) {
		$.ajax({
			url: mainWeb + '/front/getVantageGoodsList',
	...
	var renderData = function(resultData) {
		...
		for (var index = 0; index < resultData.length; index++) {
			...
			html += "<div>";
			html += "<a class='goodsCellImage' target='_blank' href='" + mainWeb + "/front/toGoodsInfoPage?sign=2&id=" + resultData[index].recid + "'";
			html += " target='_blank'><img width='130px' height='130px'";
			html += " src='" + mainWeb + resultData[index].picturepath1 + "' /></a>";
			html += "</div>";
			html += "<div class='p-name'>";
			html += "<a target='_blank' href='" + mainWeb + "/front/toGoodsInfoPage?sign=2&id=" + resultData[index].recid + "'";
			...
			html += "<strong>" + resultData[index].vantagesCost.toFixed(0) + "积分/" + resultData[index].goodsunit + "</strong>";
			html += "</div>";
			html += "<div class='bellowGoodsSmallInfo'>";
			html += "<img onclick=\"_vantageApp.adjustGoodsCount('" + resultData[index].recid + "','-1') \"";
			...
			html += " onchange=\"_vantageApp.countChangeListener('txt_" + resultData[index].recid + "')\">";
			html += "<img onclick=\"_vantageApp.adjustGoodsCount('" + resultData[index].recid + "','1') \"";
			html += " src='" + mainWeb + "/images/page/button_sum.png'";
			html += " class='bellowGoodsSmallInfo_img_add' /> ";
			html += "<img onclick=\"_vantageApp.buyGoods('" + resultData[index].recid + "') \"";
