...
function GoodsCategory() {
		...
		*/
		$("#Button_Save_E").click(function() {
			...
			$.ajax({
				...
				data: {"categoryId":selectedNode.id, "categoryName":categoryName},
				dataType: 'json',
	...
	this.nodeSelectionAction = function(node) {
		...
		if (node.id == "root" || _goodsCategoryTree.getChildren(node).length > 0) {
...
function CategoryCreateWindow() {
	...
	var renderInit = function() {
		...
		var $codeRow = $('<p><label>分类编号:</label></p>');
	...
	var pColumns = [[
		{
	    	...
	    	field:'action',
		        	...
		        	formatter : function(value, row, index) {
								if (row.editing) {
									var s = "<a href='#' onclick=\"saveRowAction('Table_Properties', " + index + ")\">保存</a> ";
									var c = "<a href='#' onclick=\"cancelEditRowAction('Table_Properties', " + index + ")\">取消</a> ";
									return s + c;
								} else {
									var e = "<a href='#' onclick=\"editRowAction('Table_Properties', " + index + ")\">编辑</a> ";
									var d = "<a href='#' onclick=\"deleteRowAction('Table_Properties', " + index + ")\">删除</a> ";
									return e + d;
								}
							}
	    }
...
-->
<div id="editWin" class="easyui-window" collapsible='false' modal='true' resizable='false' minimizable='false' maximizable='false' title="修改分类" style="width:300px;height:180px;" closed="true">  
    <form style="padding:10px 20px 10px 40px;">  
