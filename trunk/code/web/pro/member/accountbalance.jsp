...
.pagin a, .pagin span {
	...
	*height: 22px;
		...
		<script type="text/javascript">
function toCardChargePage() {
	this.location = mainWeb+'/pro/member/charge.jsp';
}

function toBankChargePage(){
	this.location = mainWeb+'/pro/member/onlineCharge.jsp';
} 
</script>
	</head>
		...
		<div id='chargecontent' class='w'>
			...
			<div class="main">
				<div class="breadcrumb">
					<span class="my7"><strong><a href="#">我的7号</a> </strong> </span><span>&nbsp;&gt;&nbsp;账户余额</span>
				</div>
				<div class='left'>
					...
					<div class="balancetop">
						<div class="topleft">
							...
							<strong class="ftx-01">￥<label id="lblBalance">
									<%=request.getAttribute("moneyBalance")%>
								</label>&nbsp;&nbsp; </strong> 帐户状态：
							<strong class="ftx-01"><label id="lblStatus">
									<%=request.getAttribute("memberStatus")%>
								</label> </strong>
						</div>
						<div class="topright">
							<span id="toCardChargePageSpan" class="btn btncharge"
								onclick="toCardChargePage();" onselectstart="return false">&nbsp;&nbsp;面值卡充值</span>
							<span id="toBankChargePageSpan" class="btn btncharge"
								onclick="toBankChargePage();" onselectstart="return false">&nbsp;&nbsp;在线支付充值</span>
						</div>
					</div>
					<div class="tableArea">
						<table id="dealingLogTable" border="0" width="100%"
							cellspacing="0" cellpadding="0" class="table_a">

						</table>
					</div>
					<div id="pagin_div"  style='height: 30px;text-align: right;width: 100%; padding-top: 10px;*padding-top: 0px;'class="pagin pagin-m fr"></div>
				</div>
			</div>
			<div id="serviceFloors" style="margin-top: -10px;">
	...
	<script type="text/javascript">
			var loadPage = function (pageIndex){
			 		var table = $('#dealingLogTable');
					table.html('<tr class="thead-tbl-grade" height="27px;"><th>序号</th><th>交易类型</th><th>收入金额</th><th>支出金额</th><th>相关数据</th><th>发生时间</th></tr>');
			 		$.post(mainWeb+'/front/getBalanceLogs',{pageIndex:pageIndex},function(result){
			 		result = eval('('+result+')');
			 		for(var i=0;i<10;i++){
			 			if(result.returnObj&&i<result.returnObj.length){
				 			...
				 			var tr = '<tr>'
				 			tr = tr + '<td>'+(i+1)+'</td>';
				 			tr = tr + '<td>'+form.dealType+'</td>';
				 			tr = tr + '<td>'+form.sumAmount+'</td>';
				 			tr = tr + '<td>'+form.subAmount+'</td>';
				 			tr = tr + '<td>'+form.relaBillsNo+'</td>';
				 			tr = tr + '<td>'+form.date+'</td>';
				 			tr = tr + '</tr>'; 
				 			table.append(tr);
			 			}else{
